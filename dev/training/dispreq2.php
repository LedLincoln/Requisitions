...
if(array_key_exists('id', $_GET)) {
  ...
  if($rid !== 0) {
    ...
    if($status == "New Quotation" or $status == "Quotation - Pending" or $status == "Quotation - Complete") {
      $title = "Training Quotation Q" . $rid;
    } else {
      $title = "Training Requisition R" . $rid;
    }
    ...
    $reqdate = $now;
    $title = "New Training Requisition";
    $status = "New";
  }
} else {
  $rid = '0';
...
<div class='eight columns header'><em>Training</em></div>
<?php
//if($dn == $createdby or $dn == $onbehalf or in_array($dn, $approvers) or in_array_any(["Buyer","Admin"], $userroles)) {
if((($status == '' or $status == 'New' or $status == 'New - Not Submitted' or $status == 'Submitted for Approval' or $status == "Quotation - Pending") and $dn == $createdby or $dn == $onbehalf) or in_array($dn, $approvers) or in_array_any(["Buyer","Admin"], $userroles)) {
  echo "<div class='two columns jr'><a class='lbut' href='edreq.php?id=$rid'>Edit <i class='material-icons md-22' data-content='Edit'>edit</i></a></div>\n";
}
?>
</div>
<div class='row'>
...
echo "<div class='reqsec $rstyle'>\n";
echo "<div class='row'>\n";
//echo "<div class='eight columns lpad'><div class='labl'>Internal comments</div><div class='dval'><pre>$comments</pre></div></div>\n";
//echo "<div class='eight columns lpad'><div class='labl'>Internal comments</div><pre class='dval'>$comments</pre></div>\n";
