...
foreach($projs as $p) {
  ...
  echo "<tr class='projrow' data-id='$proj'><td>&gt;$proj</td><td>$pname</td><td>$count</td><td class='money'>$total</td></tr>\n";
}
?>
</tbody>
</table>
...
$(".projrow").on('click', function() {
  ...
  //$(".projrow[data-id='" + projnum + "']").after('Buffalo');
  //$(".projrow[data-id='" + projnum + "']").next(".detail").remove();
  $(".projrow[data-id='" + projnum + "']").next(".detail").remove();
  $("#projtable .detail").remove();
  console.log(projnum);
  ...
  $.ajax({
    ...
    success: function(result) {
      ...
      $.each(jresult, function(k, v) {
        var rowdata = "<tr class='detail'><td><a href='dispreq.php?id=" + v['id'] + "' target='_blank'>" + v['id'] + "</a></td><td>" + v['onbehalf'] + "</td><td>" + v['description'] + "</td><td class='money'>" + v['total'] + "</td></tr>";
        $(".projrow[data-id='" + projnum + "']").after(rowdata);
      });
    }
  });
});
