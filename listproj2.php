...
<div class='lably'>Fiscal year: </div>
<select id='fy'>
...
for($k = 1; $k < 13; $k++) {
  $mo = $months[$k];
  if($k == $fyn) {
    echo "<option value='$k' selected>$mo</option>";
  } else {
    echo "<option value='$k'>$mo</option>";
  }
}
?>
</select>
  ...
  activate: function (e, ui) {
    ...
    if(ui.newHeader.length > 0) {
      ...
      if(! already_loaded[ui.newHeader[0].id] == 1) {
        ...
        var url = "getprojreqs2.php";
        var pdata = {'projnum': projnum, 'fiscalyear': fiscalyear};
        $.post(url, pdata)
...
$("#fy").on('change', function() {
  ...
  var url = "listproj2.php?fy=" + $(this).val();
